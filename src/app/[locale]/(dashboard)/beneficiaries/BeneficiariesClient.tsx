...
import { useBeneficiaries, useBilling, useInvalidateData } from '@/hooks/useDataQueries';
...
export default function BeneficiariesClient() {
    ...
    const { data: beneficiaries = [], isLoading: queryLoading } = useBeneficiaries({ enabled: dataEnabled });
    const { data: billing } = useBilling({ enabled: dataEnabled });
    const { invalidateBeneficiaries, invalidateBilling } = useInvalidateData();
    const loading = securityLoading || (dataEnabled && queryLoading);
    ...
    const limitReached = useMemo(() => {
        ...
        const max = billing.limits.maxBeneficiaries;
        if (max === -1) return false;
        return billing.usage.beneficiariesCount >= max;
    }, [billing]);
    ...
    const limitInfo = useMemo(() => {
        ...
        return {
            ...
            current: billing.usage.beneficiariesCount,
        };
