...
const AddAssetModal = dynamic(() => import('@/components/AddAssetModal'), { ssr: false });
const AssetDetailsModal = dynamic(() => import('@/components/AssetDetailsModal'), { ssr: false });
const AssetAttachmentsModal = dynamic(() => import('@/components/AssetAttachmentsModal'), { ssr: false });
...
import { useAssets, useBeneficiaries, useBilling, useInvalidateData } from '@/hooks/useDataQueries';
...
export default function DigitalAssetsClient() {
  ...
  const dataEnabled = !locked && !securityLoading;
  const { data: rawAssets = [], isLoading: assetsLoading } = useAssets({ enabled: dataEnabled });
  const { data: beneficiaries = [], refetch: refetchBeneficiaries } = useBeneficiaries({ enabled: dataEnabled });
  const { data: billing } = useBilling({ enabled: dataEnabled });
  const { invalidateAssets, invalidateBilling } = useInvalidateData();

  const assets = useMemo(
    () =>
      rawAssets.map((asset) => ({
        ...asset,
        beneficiary: Array.isArray(asset.beneficiary)
          ? asset.beneficiary[0] || null
          : asset.beneficiary || null,
      })),
    [rawAssets]
  );

  const loading = securityLoading || (dataEnabled && assetsLoading);
  ...
  const limitReached = useMemo(() => {
    ...
    const max = billing.limits.maxAssets;
    if (max === -1) return false;
    return billing.usage.assetsCount >= max;
  }, [billing]);

  const limitInfo = useMemo(() => {
    if (!billing || !limitReached) return null;
    return {
      limit: billing.limits.maxAssets,
      current: billing.usage.assetsCount,
    };
      ...
      <div className="p-4 sm:p-8">
        ...
        {selectedAssetForAttachments && (
          <AssetAttachmentsModal
            ...
            onFilesUpdated={refreshAssets}
          />
        )}
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-foreground dark:text-slate-700 sm:text-4xl">{t('digitalAssetsTitle')}</h1>
        ...
        <Dialog open={assignModalOpen} onOpenChange={(open) => { setAssignModalOpen(open); if (!open) { setSelectedAsset(null); setSelectedBeneficiaryId(null); } }}>
          <DialogContent className="max-w-lg w-full bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-700 p-4 sm:p-8">
            ...
            <DialogFooter className="mt-6 gap-2 sm:gap-2">
              <Button variant="destructive" onClick={() => handleAssignBeneficiary(null)} disabled={selectedBeneficiaryId === null || assigning} className="w-full sm:w-auto">
                {assigning ? t('saving') : t('removeBeneficiary')}
              </Button>
              <Button variant="outline" onClick={() => setAssignModalOpen(false)} className="w-full sm:w-auto">{t('cancel')}</Button>
              <Button onClick={() => handleAssignBeneficiary(selectedBeneficiaryId)} disabled={!selectedBeneficiaryId || assigning} className="w-full sm:w-auto">
                {assigning ? t('saving') : t('assignBeneficiary')}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
